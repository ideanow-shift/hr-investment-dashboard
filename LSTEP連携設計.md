...
2. データ更新後、LSTEP連携状況が「LSTEP受け皿あり」になることを確認する
3. LSTEP未紐付けCSVを出力し、LSTEP側の友だち情報と照合する
4. CSV/手動同期で `talent_line_accounts` に紐付けデータを登録する
5. GAS定期同期またはLSTEP API/Webhook連携方式を決める
6. 同期実行ログと失敗時の再実行導線を整える
